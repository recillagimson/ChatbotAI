  ...
  try {
    const { text: out } = await openaiChat({
      ...
      system:
        ...
        "- bot            = the sender is itself an automated bot or a spammer with NO genuine interest: word-salad/gibberish, an UNSOLICITED promotional or mass-DM blast, or canned non-sequiturs that do NOT track the conversation. A real person is NOT a bot just because they paste a link (e.g. sharing their OWN website or company site) or re-send the same message after getting no reply - a relevant URL or a repeat ALONE is never 'bot'. Flag 'bot' ONLY when the message is spam AND shows no genuine interest in the service.\n" +
        "- none           = anything else. A real person with ANY genuine interest, questions, negotiation, venting, or frustration - EVEN with heavy profanity, EVEN if they paste a relevant link or re-send the same message - who still wants help is 'none'.\n" +
        ...
        "Be very conservative - answer 'abusive', 'not_interested', or 'bot' ONLY when it is UNMISTAKABLE; when in doubt, none. Reply with ONLY the one word.",
      messages: [
        {
          role: "user",
