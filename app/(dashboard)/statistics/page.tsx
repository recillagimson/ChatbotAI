...
import { formatSecs, num, pct as fmtPct, tickDate } from "@/lib/format";
 ...
 * scope - only changes the query string. Without a keyed boundary those clicks
 * would sit on the old numbers with no feedback for as long as the report takes.
 * Keying the Suspense on the params makes React throw the subtree away and show
 * the skeleton the moment the URL changes. (Funnel stage drill-down is NOT one of
 * these: it loads client-side and never re-renders the report.)
 *
...
export default async function StatisticsPage({
  ...
  const reportKey = [sp.range, sp.from, sp.to, sp.bot].join("|");
        ...
        actions={
          <>
            <Suspense
              fallback={
                <Sk
                  className="h-[38px] w-[124px] rounded-ctl-lg"
                  tone="on-page"
                />
              }
            >
...
async function StatisticsReport({ sp }: { sp: StatsParams }) {
  ...
  let botsQuery = supabase
    .from("chatbots")
    .select(
      "id, auto_followup_enabled, auto_followup_steps, auto_followup_link_steps",
    )
    .eq("user_id", user!.id);
    ...
    <>
      ...
      {overview && overview.usage.delivery_failures > 0 && (
        ...
        >
          ...
          of attempted sends in this period failed - most of them were outside
      ...
      {overview && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          <StatCard
            ...
            foot={
              prev ? `${num(prev.funnel.entry)} last period` : "no prior period"
            }
          />
          <StatCard
            label="AI replies"
          ...
          <StatCard
            ...
            value={fmtPct(
              safePct(overview.funnel.replied, overview.funnel.entry),
            )}
            valueTone="green"
          ...
          {overview.series.length === 0 ? (
            ...
            <div className="mt-[18px] flex gap-3.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                />
                <AxisTicks
                  labels={axisLabels(overview.series.map((d) => d.day))}
                />
              </div>
            </div>
          )}
          ...
          {!funnel ? (
            <>
              ...
              <p className="mt-4 text-[12.5px] leading-relaxed text-ss-muted">
                The funnel comes from the analytics report above, so it fills in
                once that loads.
              </p>
            </>
          ) : (
            <>
              {/* Stage drill-down is client-side: opening a stage loads only
                        its members and every stage preloads after mount, so the
                        report never re-renders. */}
              <InboundFunnel
              ...
              {disqualified > 0 && (
                <div className="mt-4 flex flex-wrap items-center gap-4 rounded-panel border border-ss-rose-line bg-ss-rose-tint px-5 py-4">
                  ...
                  <p className="max-w-[19rem] text-[12.5px] leading-relaxed text-ss-rose-ink">
                    Tagged disqualified and out of play. They are still counted
                    in the stages above - the funnel measures how far threads
                    got, not how many were worth having.
                  </p>
                  <SsLinkButton
      ...
      <SsCard className="p-6">
        ...
        {!sequences.preLink.length && !sequences.postLink.length ? (
          <p className="mt-4 text-[12.5px] leading-relaxed text-ss-muted">
            ...
            {chatbotId ? "this chatbot" : "any chatbot"} yet, so a thread that
            goes quiet stays quiet. Steps are set per chatbot on its Follow-ups
            tab.
          </p>
        ) : (
          <>
            ...
            <p className="mt-4 border-t border-ss-hair pt-4 text-[12px] leading-relaxed text-ss-muted">
              ...
              there&apos;s one timestamp per thread, not one per send - so a
              reply-per-step rate would be invented rather than measured.
              {sequences.mixed
      ...
      >
        ...
        phases - SpeedSettr doesn&apos;t record one, so there is nothing behind
        that card yet. It comes back when the bot writes a phase per thread.
      </EmptyState>
    </>
  );
}
