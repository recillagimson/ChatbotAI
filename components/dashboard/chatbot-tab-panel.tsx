...
import { GraduationCap, Key, TriangleAlert, Zap } from "lucide-react";
...
import { type ChatbotTabKey } from "@/lib/chatbot-tabs";
...
/**
 * One chatbot management tab's contents - the shared body behind BOTH the client
 * dashboard page (app/(dashboard)/chatbots/[id]/page.tsx) and the superadmin admin
 * route (app/(admin)/admin/clients/[clientId]/bots/[botId]/page.tsx).
 *
 * Parameterized instead of reading getCurrentUser():
 *  - `ownerId` is the bot's OWNER (the client). On the client page it's the logged-in
 *    user; on the admin route it's the client being managed. Every owner-scoped read
 *    and the asset-manager's upload folder use it, NOT the caller - so a superadmin
 *    manages the client's real data (RLS admin overlays authorize the reads).
 *  - `basePath` is the route the tabs live on (e.g. `/chatbots/<id>` or
 *    `/admin/clients/<cid>/bots/<id>`), used for the Overview tab's cross-tab links.
 *
 * It re-reads the chatbot row (one indexed PK lookup per tab render) rather than
 * taking a dozen props from the shell.
 */
export async function ChatbotTabPanel({
  chatbotId,
  ownerId,
  basePath,
  tab,
  ...
  // AND for an admin operating this panel from /admin. Checked only on the two
  // tabs that need it to avoid the extra query elsewhere.
  const isSuperadmin =
            ...
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
              ...
              <SsCard className="p-[22px]">
                <SsCardHead
                  title="Channels"
                  action={
                    <SsCardLink href={`${basePath}?tab=connection`}>
                      Manage connection
                    </SsCardLink>
                  }
                />
              ...
              <div className="flex flex-col gap-5">
                ...
                <SsCard className="p-[22px]">
                  ...
                  <div className="mt-3.5">
                    <SsCardLink href={`${basePath}?tab=keywords`}>
...
function buildFixes(ctx: {
  ...
  if (!ctx.apiKeySet) {
    out.push({
      ...
      href: `${ctx.basePath}?tab=connection`,
      cta: "Add the key",
      urgent: true,
    });
  }
  ...
  if (ctx.trained === 0) {
    out.push({
      ...
      href: `${ctx.basePath}?tab=training`,
...
/**
 * The placeholder for a tab that's loading. Shaped per tab rather than one generic
 * block so a switch never shows the wrong silhouette.
 */
export function ChatbotTabSkeleton({ tab }: { tab: ChatbotTabKey }) {
  if (tab === "overview") {
    return (
      <>
